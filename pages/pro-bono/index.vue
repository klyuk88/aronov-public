...
const {data: pageData} = await useFetch(
  () => `/api/pro-bono-page?populate=*&locale=${locale.value}`,
  {
    baseURL: config.api
  }
)
...
const { data: cases } = await useFetch(() => `/api/pro-bonos?sort=date:desc&pagination[pageSize]=6&pagination[page]=${currentPage.value}&locale=${locale.value}`, {
...
<template>
  ...
  <section class="pro-bono pt-120 pb-120">
    <div class="container">
      <h1 class="page-title">{{pageData.data.attributes.title}}</h1>
      
      <div
        class="mt-5 text-white"
        v-if="pageData.data.attributes.content"
        v-html="pageData.data.attributes.content"
      ></div>

      <div class="row mt-120 justify-content-between pro-bono-items row-gap-5" v-if="cases.data.length > 0">
        <div
          class="col-lg-5 pro-bono-item"
          v-for="(item, index) in cases.data"
          :key="item.id"
        >
          <nuxt-link :to="`/pro-bono/${item.attributes.slug}`">
