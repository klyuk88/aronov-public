...
const { error: allMediaError, data: allMedia } = await useFetch(
  () => `/api/cases?locale=${locale.value}&sort[0]=date:asc&pagination[pageSize]=10000`,
  {
    baseURL: runtimeConfig.public.api,
  }
);
...
const nextElem = () => {
  usePageSingleNavNext(currentIndex, allMedia, "cases");
};

const prevElem = () => {
  usePageSingleNavPrev(currentIndex, allMedia, "cases");
};
</script>
<template>
      ...
      <div class="single_page-raw">
        ...
        <div class="overflow-hidden">
          <div class="d-flex align-items-center justify-content-between mb-5">
            <client-only>
              <span class="single_page-date">{{ pubDate }}</span>
            </client-only>
            <ShareBtn />
          </div>
          ...
          <h1 class="single_media-title mb-4">
            {{ singleMedia.title }}
          </h1>

          <div class="mb-4" v-if="singleMedia.practices.data">
            <h3 class="fs-5 fw-medium with-decor">
              {{$t('Practice')}}:
              <span class="fw-normal">{{
                singleMedia.practices.data.attributes.title
              }}</span>
            </h3>
          </div>

          <div class="single_page-content text-lg-left" v-html="content"></div>

          <div class="mt-5 w-100" v-if="singleMedia.teams.data.length > 0">
            <h4 class="fs-3">{{$t('Single page team title')}}</h4>
          <client-only>
            <Swiper
            class="case-page-slider mt-5"
              :modules="[Navigation]"
              :slides-per-view="3"
              :space-between="30"
              navigation
            ...
            >
              <SwiperSlide
                v-for="(item, index) in singleMedia.teams.data"
                :key="index"
              >
